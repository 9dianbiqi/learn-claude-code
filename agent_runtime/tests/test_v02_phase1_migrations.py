...
def test_fresh_store_creates_v11_directly(tmp_path: Path):
    ...
    assert [int(row["version"]) for row in versions] == [11]
...
def test_repeated_migrate_is_idempotent_and_does_not_add_a_second_audit_row(tmp_path: Path):
    ...
    assert second.from_version == 11
    assert second.to_version == 11
...
def test_future_schema_version_fails_closed(tmp_path: Path):
    ...
    with sqlite3.connect(database) as connection:
        ...
        connection.execute(
            "INSERT INTO schema_migrations(version, name, checksum, applied_at) VALUES (12, 'future', 'future', 0)"
        )
...
def test_migration_boundary_faults_leave_a_valid_pre_or_post_state(tmp_path: Path, point: str):
    ...
    assert version == (4 if point == "after_migration_backup" else 11)
