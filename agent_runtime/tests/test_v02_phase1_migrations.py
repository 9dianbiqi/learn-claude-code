...
def test_v7_database_migrates_to_v11(tmp_path: Path):
...
def test_checksum_mismatch_fails_closed(tmp_path: Path):
    ...
    with sqlite3.connect(store.path) as connection:
        connection.execute(
            "UPDATE schema_migrations SET checksum = 'tampered' WHERE version = 12"
        )
        connection.commit()
...
def test_repeated_migrate_is_idempotent_and_does_not_add_a_second_audit_row(tmp_path: Path):
    ...
    assert second.from_version == 12
    assert second.to_version == 12
    assert second.applied == ()
    with sqlite3.connect(database) as connection:
        assert connection.execute("SELECT COUNT(*) FROM schema_migrations").fetchone()[0] == 9


def test_v5_database_migrates_to_v11(tmp_path: Path):
...
def test_future_schema_version_fails_closed(tmp_path: Path):
    ...
    with sqlite3.connect(database) as connection:
        ...
        connection.execute(
            "INSERT INTO schema_migrations(version, name, checksum, applied_at) VALUES (13, 'future', 'future', 0)"
        )
    with pytest.raises(UnsupportedSchemaVersion):
        SchemaManager(database).inspect()


def test_concurrent_migrate_has_one_writer_and_one_final_v11_reader(tmp_path: Path):
    database = _v4_database(tmp_path, ("completed", "unknown"))
...
def test_migration_boundary_faults_leave_a_valid_pre_or_post_state(tmp_path: Path, point: str):
    ...
    assert version == (4 if point == "after_migration_backup" else 12)
